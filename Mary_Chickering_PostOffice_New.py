    ...
    while True:                                                                        #defines main function
        ...
        if len(dimensions) != 5:                                                       #if the length of the list does not equal 5
            ...
            try:                                                                       #attempts the following code to check for errors in user input
                ...
                formatted_cost = ("{:.2f}".format(total_cost).lstrip("0"))             #formatted cost is the total cost but the zero before the decimal point is stripped and adds the trailing zero if theres only one sig fig on the left of the decimal
